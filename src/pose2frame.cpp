 ...
 * @param input PFで得られた姿勢.frame_idは"map"であることが前提．姿勢が３次元の場合はz=0平面にプロジェクションするかを選択
 */
...
void pose_callback(const nav_msgs::Odometry::ConstPtr &arg) {
  ...
  try {
    ...
    ROS_INFO_STREAM("mapPoseTrans x:" << mapPoseTrans.getOrigin().getX()
                                      ...
                                      << " yaw:" << tf2::getYaw(mapPoseTrans.getRotation()));
    // "base_footprint" -> "odom"
    odomBaseTransMsg = tfBuffer_.lookupTransform("odom", "base_footprint", /*ros::Time::now()*/ arg->header.stamp,
                                  ros::Duration(3.0));
